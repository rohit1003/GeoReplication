...
void periodic_pull(int Lead_N1_Port,string Lead_IP){
	...
	while(1){
		string msg="";
		msg = msg + "1#CHG#"; 	/* 1 is to indicate Replica Node 1,so that leader read its Queue 1 */
...
int main(){
	...
	cout<<"\nDC1: Replica Node : 1 started ..";
	int Lead_Repl_Port = stoi(config_read("DC1_PORT_LEAD_REPL")); /* 5100 */
	string Lead_IP = config_read("DC1_LEAD_IP");

	int Repl_Port = stoi(config_read("DC1_PORT_REPL1_LEAD"));  /* 5001 */
	//int self_port = 5001; 
